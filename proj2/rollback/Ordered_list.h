...
Ordered_list<T, OF>::Ordered_list(const Ordered_list& original)
    : list_size{original.list_size}, head{nullptr}, tail{nullptr} {
  // I don't really understand why the compiler balks at performing this initialization
  // in brackets, but it does.
...
Ordered_list<T, OF>::Ordered_list(Ordered_list&& original)
    ...
    head{original.head},
    tail{original.tail} {
  // Again, compiler balks at this in brackets.
  ordering_f = original.ordering_f;
...
void Ordered_list<T, OF>::swap(Ordered_list & other) noexcept {
  swap_helper(head, other.head);
  swap_helper(tail, other.tail);
  swap_helper(list_size, other.list_size);

  // The compiler doesn't like passing these by reference and then constructing a temp in
  // swap_helper.
  OF temp = other.ordering_f;
  other.ordering_f = ordering_f;
  ordering_f = temp;
}
