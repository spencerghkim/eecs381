...
struct Ordered_container* OC_create_container(OC_comp_fp_t comp_func)
{
  struct Ordered_container* c_ptr =
      (struct Ordered_container*)safe_malloc(sizeof(struct Ordered_container));
...
void OC_apply(const struct Ordered_container* c_ptr, OC_apply_fp_t afp)
{
  ...
  /* TODO(wjbillin): Is it worth getting the size just to avoid deref? */
  int size = c_ptr->size;
...
void grow_array_helper(struct Ordered_container* c_ptr) {
  int new_allocation;
  void** new_array;
  int i;
  ...
  new_array = (void **)safe_malloc(new_allocation * sizeof(void *));
...
void initialize_array_helper(struct Ordered_container* c_ptr)
{
  c_ptr->array = (void **)safe_malloc(INITIAL_ARRAY_SIZE * sizeof(void *));
